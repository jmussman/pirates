...
const router = () => {
    ...
    app.get('/ships', async (req, res) => {
        ...
        if (!req.headers.authorization || await validate(req.headers.authorization, apiAudience, 'read:ships')) {
    ...
    app.get('/treasure', async (req, res) => {
        ...
        // Extract the token from the header, if it isn't there or not valid send back 401.

        if (req.headers.authorization && await validate(req.headers.authorization, apiAudience, 'read:treasure')) {
    ...
    console.log(`Web service listening on port ${apiPort}`)
    app.listen(apiPort)
}
